...
http.listen(3000, function (e) {
    console.log('listening on *:3000',e);
});
function currRoomID() {
    return ("000000" + roomCount).substr(-6);
}
//生成房间号
function getRoomID() {
    ...
    let roomID = currRoomID();

    let room = roomMap[roomID];

    if (room && room.playerList.length == 3) {
        roomCount++;
    }
    return currRoomID();
}
...
io.on('connection', function (socket) {
    ...
    socket.on('disconnect', function () {
        ...
        //下次创建的是新房间
        roomCount++;
        console.log("重新设置房间序号:",roomCount);
    });
    //进入大厅,系统分配房间号
    socket.on('enterGame', function (args) {
        let user = args.user;
        let isCreateRoom = false;
        if (roomCount == 0) roomCount++;
        let room = roomMap[currRoomID()];
        if (room) {
        ...
        if (isCreateRoom) {
            ...
            console.log("创建房间:" + roomID, "用户名:" + user);
            ...
            console.log("现有房间:" + roomID, "用户名:" + user);
        }
        ...
        broadCast("getRoomDataBack", { players: players, roomID: roomID }, room);
        //socket.emit('enterGameBack', {roomID:roomID,user:user});
    });
    //创建房间号
    ...
    //准备
    socket.on('readyGame', function (args) {

        var roomNum = args.roomID;
        var roomIndex = args.index;
        ...
        broadCast('readyGame' + roomNum, { roomIndex: roomIndex, ready: player.isReady }, room);
    ...
    socket.on("getCards", function (arg) {

        var roomNum = arg.roomID;
